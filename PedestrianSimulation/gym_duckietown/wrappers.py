...
class ResizeWrapper(gym.ObservationWrapper):
    ...
    def reset(self):
        ...
        return cv2.resize(obs.swapaxes(0, 2), dsize=(self.resize_w, self.resize_h),
                          interpolation=cv2.INTER_CUBIC).swapaxes(0, 2)

    def step(self, actions):
        import cv2
        obs, reward, done, info = gym.ObservationWrapper.step(self, actions)
        return cv2.resize(obs.swapaxes(0, 2), dsize=(self.resize_w, self.resize_h),
                          interpolation=cv2.INTER_CUBIC).swapaxes(0, 2), reward, done, info
